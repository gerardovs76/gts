 ...
 */
 ...
 dd(json_encode($notas));
    ...
    public function enviarLibretasIndividuales()
    {
        return view('notas.enviar-libreta-individual');
    }
    public function storeLibretaIndividualEnviar($curso, $paralelo, $parcial, $quimestre)
    {
        $matriculados = Matriculacion::join('inscripciones', 'matriculados.cedula', '=', 'inscripciones.cedula')
        ->where('matriculados.curso', $curso)->where('matriculados.paralelo', $paralelo)
        ->select(DB::raw("CONCAT(matriculados.apellidos, ' ',matriculados.nombres) as nombres"), 'matriculados.cedula', 'inscripciones.email')
        ->orderBy('matriculados.apellidos')
        ->get();

        return response()->json($matriculados);
    }
    public function enviarLibretaIndividualDefinitiva($cedula, $email, $parcial, $quimestre)
    {
        
        $matriculadosInfo = Matriculacion::where('cedula', $cedula)
        ->select(DB::raw("CONCAT(apellidos,' ',nombres) as nombres"), 'curso', 'paralelo')
        ->get();
        $materias = Materias::join('matriculados as m1', 'materias.curso', '=', 'm1.curso')->join('matriculados as m2', 'materias.paralelo', '=', 'm2.paralelo')->where('m1.cedula', $cedula)->where('m2.cedula', $cedula)->select('materias.materia','materias.id', 'materias.tipo_materia')->distinct()->get();
        $notas = Matriculacion::with(['notas_ta' => function($query1) use($parcial, $quimestre){
            $query1
            ->where('parcial', $parcial)
            ->where('quimestre', $quimestre)
           ...
           $pdf = PDF::loadView('pdf.libreta-individual', compact('notas','inspe','materias', 'notasPromedioFinalTa', 'notasPromedioFinalTi', 'notasPromedioFinalTg', 'notasPromedioFinalLe', 'notasPromedioFinalEv', 'parcial', 'quimestre','representante'))->stream();
           if($email != '')
           {
    ...
    public function libretaIndividualQuimestreStore(Request $request)
    {
        ...
        $notas = Matriculacion::with(['notas_ta1' => function($query) use($quimestre){
            ...
            $inspe2 = Matriculacion::withCount(['inspecciones as h1_count_01' => function($query) use($quimestre){
                $query
                ->where('parcial', '2')
                ->where('quimestre', $quimestre)
                ->where('h1', '01');
            ...
            $inspe3 = Matriculacion::withCount(['inspecciones as h1_count_01' => function($query) use($quimestre){
                $query
                ->where('parcial', '3')
                ->where('quimestre', $quimestre)
                ->where('h1', '01');
    ...
    public function libretasQuimestralesProfesores()
    {
        return view('notas.libretas-especificas-profesores');
    }
    public function descargarLibretaColectivaParaProfesores(Request $request)
    {
        $curso           = $request->curso;
        $paralelo        = $request->paralelo;
        ...
        $materias_id     = [];
        $materias    = [];
        $profesor_materias = Profesor::join('users', 'profesors.cedula', '=', 'users.cedula')
        ->where('profesors.cedula', $profesor_cedula)
        ->select('profesors.id')
        ->get();
        foreach($profesor_materias as $profesor)
        {
            $all_materias_profesor = MateriasProfesor::where('profesores_id', $profesor->id)
            ->select('materias_id')
            ->get();
            foreach($all_materias_profesor as $materias_profesor)
            {
                $materias_id[] = $materias_profesor->materias_id;
            }
        }
